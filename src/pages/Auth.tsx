...
import { getStableAuthErrorMessage } from '@/services/authErrorMessageService';
...
} from '@/services/socialLoginPreferenceService';
...
function endpointError(payload: unknown, fallback: string): Error {
...
export default function Auth() {
  ...
  const [authScreenSeenBefore] = useState(() => hasSeenAuthScreen());
  const [lastOAuthProvider, setLastOAuthProvider] = useState<DesktopOAuthProvider | null>(() => readLastOAuthProvider());
  ...
  const handleSignup = async (data: SignupFormData) => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: t('common.error'),
        description: getStableAuthErrorMessage(error, t),
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdatePassword = async (data: UpdatePasswordFormData) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: t('common.error'), description: getStableAuthErrorMessage(error, t) });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOAuth = async (provider: DesktopOAuthProvider) => {
    setLoading(true);
    rememberLastOAuthProvider(provider);
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8 text-center lg:text-left">
            <h1 className="text-2xl font-semibold text-foreground mb-1">
              {mode === 'login' ? t(authScreenSeenBefore ? 'auth.login.titleReturning' : 'auth.login.titleFirstVisit')
            ...
            <p className="text-sm text-muted-foreground">
              {mode === 'login' ? t(authScreenSeenBefore ? 'auth.login.subtitleReturning' : 'auth.login.subtitleFirstVisit')
            ...
            {(mode === 'login' || mode === 'signup') && (
              <>
                <div className="grid grid-cols-3 gap-3 mb-6">
                  <Button variant="outline" onClick={() => handleOAuth('google')} disabled={loading} className="flex h-auto min-h-10 w-full flex-col gap-1 py-2" aria-label="Google Login" title="Google Login">
                    ...
                    {lastOAuthProvider === 'google' && (
                      <span className="text-[10px] leading-none text-muted-foreground">{t('auth.providers.lastUsed')}</span>
                    )}
                  </Button>
                  <Button variant="outline" onClick={() => handleOAuth('discord')} disabled={loading} className="flex h-auto min-h-10 w-full flex-col gap-1 py-2" aria-label="Discord Login" title="Discord Login">
                    ...
                    {lastOAuthProvider === 'discord' && (
                      <span className="text-[10px] leading-none text-muted-foreground">{t('auth.providers.lastUsed')}</span>
                    )}
                  </Button>
                  <Button variant="outline" onClick={() => handleOAuth('github')} disabled={loading} className="flex h-auto min-h-10 w-full flex-col gap-1 py-2" aria-label="GitHub Login" title="GitHub Login">
                    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
