...
Baseline commits: `b3a27c3` (`Harden CORS and remove leaked loadtest artifacts`), `810bc35` (`Harden vault metadata neutralization and Tauri opener scope`)
...
- `b3a27c3` and `810bc35` are contained in the current branch.
- `810bc35` was already pushed to `origin/feat/zero-knowledge.hardering` at the start of this finalization pass.
- The working tree was clean at the start of this finalization pass.
...
- On the first safe unlocked Vault load, decrypt `encrypted_data`. If payload lacks fields that only exist in legacy columns, merge legacy metadata into the decrypted payload locally.
...
- Added `legacyVaultMetadataMigrationService.ts` as the idempotent per-item migration runner. It merges legacy server-visible metadata into the decrypted payload locally, re-encrypts with the current item envelope, updates only `encrypted_data` plus neutral server fields, binds remote updates by `id` and `user_id`, and refreshes the offline cache.
...
- Category legacy encryption still runs in the existing category load paths. A future cleanup should move category migration into the same service family before removing category fallback reads.
...
| `account-delete` | Yes | Destructive account deletion | Bearer user auth + RPC owner checks | `auth.getUser(accessToken)`, user-bound RPC | `account_delete` top-level throttle plus RPC/2FA path | POST-only shared allowlist | service role for storage cleanup only after auth | Destructive endpoint | Hardened |
...
| `webauthn` | Yes | Passkey operations | Action-specific user session | `authorizeWebauthnAction`, bearer extraction, challenge scope binding | `webauthn_challenge`, `webauthn_verify`, `webauthn_manage` | shared allowlist | service role for challenges/credentials | Passkey abuse if challenge scope regresses | Hardened |
| `admin-team` | No longer listed in Open-Core config | Team/admin private area | Private admin auth required in Premium/private deployment | No public source present | Not present in Core | Not present in Core | Unknown | Dangerous if implemented as open stub later | Removed from Core deploy config |
| Premium entries | No longer listed in Open-Core config | Billing/support/family/release | Private/premium-specific | No public source present | Not present in Core | Not present in Core | Unknown | Must not ship as open admin/support endpoints | Removed from Core deploy config |
...
| FU-2026-04-28-03 | P2 | Legacy migration | Full client-side legacy metadata migration remains incomplete. | Item migration runner implemented; category service cleanup remains P3 |
| FU-2026-04-28-04 | P2 | Edge Functions | `admin-team` and premium functions are listed in public `config.toml` with `verify_jwt=false` but source is absent from public core. | Fixed: removed from Core config and documented Premium boundary |
| FU-2026-04-28-05 | P2 | Edge Functions | `account-delete` and `webauthn` rely on auth/challenge checks but do not show a uniform top-level rate-limit policy for every action. | Fixed with shared rate-limit actions |
...
| FU-2026-04-28-08 | P2 | Tauri | Default capability still allowed app-log read although frontend only writes diagnostics. | Fixed: removed `fs:allow-applog-read` |
...
- `src/services/legacyVaultMetadataMigrationService.test.ts`
...
- `npm run test` passed. Existing React `act(...)`, React Router future-flag and expected test-log warnings remain.
...
- `npm run test -- --run src/test/security-regression-suite.test.ts src/test/edge-cors-contract.test.ts src/test/account-delete-runtime-hardening.test.ts` passed: 29 tests.
- `npm run test -- --run src/services/vaultMetadataPolicy.test.ts src/services/__tests__/offlineVaultService.test.ts src/test/security-hardening-contracts.test.ts src/test/tauri-capability-contract.test.ts src/test/unit-pure-functions.test.ts src/services/legacyVaultMetadataMigrationService.test.ts src/test/account-delete-runtime-hardening.test.ts src/test/webauthn-auth-policy.test.ts` passed: 117 tests.
- `npm audit --omit=dev` passed with 0 vulnerabilities.
- `npx tsc --noEmit` passed.
- `supabase migration list` showed the new `20260428170000` migration as local and not yet remote-applied.
- `supabase db lint` ran against the local schema and reported pre-existing errors in `public.generate_share_token` and `public.match_reflections`.
- `supabase migration up --local` could not run because the local migration history contains remote versions that are not present in this local migrations directory; no repair was attempted.
- `supabase status -o env` could not inspect the local DB container (`supabase_db_lcrtadxlojaucwapgzmy` missing), so the SQL verification file was updated but not executed against a running local database.
...
- Runtime check opened `http://127.0.0.1:5177/vault/settings`; unauthenticated routing redirected to `/auth?redirect=%2Fvault%2Fsettings` without Provider, invalid-hook or duplicate-module console errors. The only error was the expected unauthenticated `auth-session` 401.
