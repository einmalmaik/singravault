...
Accepted as the target integrity architecture. The current implementation provides the V2 service layer and migration gates while preserving the R3 compatibility path for vaults that still contain legacy item envelopes.
...
The service layer is ready for V2-native vaults and controlled migration. Existing production vaults that still contain legacy `sv-vault-v1` or legacy no-AAD rows must complete explicit re-encryption before they can be represented as fully migrated Manifest V2 vaults. Runtime E2E coverage must prove the unlocked browser/Tauri flows against a provisioned test account before claiming full operational R4 for all deployed paths.
