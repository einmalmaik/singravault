-- Harden delete_my_account by requiring an explicit, short-lived
-- sensitive_action_challenge instead of relying on JWT iat freshness alone.
-- A silent session refresh mints a new JWT with a fresh iat, which previously
-- satisfied the reauthentication check without any credential verification.
-- An explicit challenge forces the caller to hold a one-time server token that
-- can only be obtained when a fresh iat is present, and it is consumed
-- atomically on use so it cannot be replayed.
...
AS $$
BEGIN
    RETURN public.issue_sensitive_action_challenge('account_delete', 180);
END;
$$;

REVOKE ALL ON FUNCTION public.begin_account_delete_challenge() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.begin_account_delete_challenge() TO authenticated;

COMMENT ON FUNCTION public.begin_account_delete_challenge() IS
    'Issues a short-lived, one-time challenge required before delete_my_account can wipe the account. Requires a fresh JWT iat (within 300 s).';

CREATE OR REPLACE FUNCTION public.delete_my_account(
    p_sensitive_action_challenge_id UUID,
...
BEGIN
    ...
    -- Consume the one-time challenge that was issued after fresh reauthentication.
    -- consume_sensitive_action_challenge raises RECOVERY_CHALLENGE_REQUIRED when the
    -- challenge is missing, expired, or does not belong to the current user.
    PERFORM public.consume_sensitive_action_challenge(
        'account_delete',
        p_sensitive_action_challenge_id
    );
...
REVOKE ALL ON FUNCTION public.delete_my_account(UUID, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.delete_my_account(UUID, UUID) TO authenticated;

COMMENT ON FUNCTION public.delete_my_account(UUID, UUID) IS
    'Atomically deletes the authenticated user account and all associated data after consuming a one-time sensitive-action challenge. The challenge must be obtained via begin_account_delete_challenge() immediately before this call.';
